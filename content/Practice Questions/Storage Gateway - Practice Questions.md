...
> > [!success]- Answer & Rationale
> > **Answer: 2, 4**
> > **Rationale**: **Choice 2**: [[Storage Gateway Overview#S3 File Gateway|S3 File Gateway]] provides an NFS/SMB interface with local caching, perfectly replacing traditional NFS storage while backing data to [[S3 Overview|Amazon S3]]. **Choice 4**: [[Storage Gateway Overview#Volume Gateway|Volume Gateway]] provides iSCSI block storage with local caching (Cached Volumes), replacing on-premises block storage. Choice 1 ([[S3 Overview|Amazon S3]] mounting) and 5 ([[EFS Overview|Amazon EFS]] over Direct Connect/VPN) typically lack the high-performance local caching provided by [[Storage Gateway Overview|AWS Storage Gateway]]. Choice 3 is for migration/data transfer, not a permanent storage replacement.
