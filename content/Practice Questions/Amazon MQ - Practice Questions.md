...
> > **Answer: 3**
> > **Rationale:** Amazon [[Amazon MQ|MQ]] natively supports industry-standard protocols and APIs like JMS, AMQP, and XA transactions (via ActiveMQ). It is designed for lift-and-shift migrations where rewriting the application code to use SQS/SNS is not feasible.
...
> > [!success]- Answer & Rationale
> > **Answer: 3**
> > **Rationale:** In an Amazon [[Amazon MQ|MQ]] Active/Standby broker deployment for ActiveMQ, the brokers share an Amazon [[EFS Overview|EFS]] (Elastic File System) storage volume. This allows the standby broker to seamlessly take over the storage state if the active broker fails.
