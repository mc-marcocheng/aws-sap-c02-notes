...
- **Destinations**: Logs can be published to **CloudWatch Logs**, **Amazon S3**, or **Amazon Kinesis Data Firehose**.
- **Limitations**:
    - Does **NOT** capture traffic generated by instances when they contact the Amazon DNS server.
    - Does **NOT** capture DHCP traffic.
    - Does **NOT** capture traffic to and from the VPC router's reserved IP address.
    - Does **NOT** capture traffic to and from `169.254.169.254` for instance metadata.

---
## Advanced Networking Services (Brief)

- **Amazon VPC IP Address Manager (IPAM)**: Centralized management of IP addresses across regions and accounts.
- **Amazon VPC Lattice**: Simplifies service-to-service communication at Layer 7.
- **AWS Network Firewall**: Managed perimeter defense with DPI and IDS/IPS.
