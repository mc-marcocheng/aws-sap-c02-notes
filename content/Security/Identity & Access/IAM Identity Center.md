...
- **Permission Sets:** A collection of IAM policies (managed or inline) that you assign to users or groups. 
    - **Mechanics**: Each Permission Set is automatically provisioned as a corresponding **IAM role** in the target AWS accounts.
...
- **Multi-Region Resiliency:** IAM Identity Center is deployed in a single region. If the IAM Identity Center region goes down, **no new SSO logins** are possible. Existing active sessions/temporary credentials may continue until expiration. 
...
> **Regional Limitation**: SSO login is a regional dependency. If the home region is down, new authentication requests fail.
