...
- **Data Sources:** Analyzes [[VPC Overview|VPC Flow Logs]], [[CloudTrail Overview|AWS CloudTrail]] event logs, CloudTrail management events, CloudTrail S3 data events, [[Route 53 Overview|Amazon Route 53]] DNS logs, EKS audit logs, and EBS volume data (for malware protection).
...
> Expect scenarios where an organization needs to auto-remediate GuardDuty findings. The pattern is always: GuardDuty -> [[EventBridge|Amazon EventBridge]] -> [[Lambda Overview|AWS Lambda]] or [[Systems Manager Overview|AWS Systems Manager Automation]].
