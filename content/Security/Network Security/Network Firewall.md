...
- **Centralized Deployment Pattern:** Often deployed in a Centralized Inspection VPC connected via [[Transit Gateway Overview|AWS Transit Gateway]]. All East-West (VPC-to-VPC) and North-South (VPC-to-Internet) traffic is routed through the Transit Gateway to the Inspection VPC, processed by the Network Firewall, and then routed to its destination.
