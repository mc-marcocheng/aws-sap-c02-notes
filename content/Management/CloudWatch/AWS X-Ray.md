...
- **AWS Lambda:** Native integration. Just enable active tracing. The Lambda platform runs the X-Ray daemon automatically.
- **Amazon ECS / EKS:** You must run the X-Ray daemon as a sidecar container in your task definition or pod.
...
**Solution:** Configure the X-Ray daemon/SDK in source accounts to assume an IAM role in a central monitoring account to write trace data. X-Ray now supports cross-account observability out-of-the-box using **CloudWatch cross-account observability**, which links a monitoring account to source accounts.
...
**Challenge:** Intermittent high latency in an ECS-based microservice environment interacting with DynamoDB and third-party APIs.
...
---
**Practice:** [[AWS X-Ray - Practice Questions|AWS X-Ray Practice Questions]]
